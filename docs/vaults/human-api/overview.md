...
- **Production:** `https://api.1claw.co` (or your Cloud Run URL, e.g. `https://oneclaw-vault-XXXX.run.app`)
- **Dashboard proxy:** The app at [1claw.co](https://1claw.co) proxies `/api/v1/*` to the same API, so from the browser the base is effectively `https://1claw.co/api`.
...
See [Authentication](/docs/vaults/human-api/authentication) for details and response shape.
...
| **Sharing**  | POST /v1/secrets/:secret_id/share, GET/DELETE /v1/share/:share_id                   | Share secrets by email or link ([guide](/docs/sharing/overview)) |
...
- Errors use **RFC 7807**-style JSON: `type`, `title`, `status`, `detail`. See [Error codes](/docs/reference/error-codes) and [Human API errors](/docs/vaults/human-api/errors).

## Next

- [Authentication](/docs/vaults/human-api/authentication) — All ways to get a JWT.
- [Secrets: create](/docs/vaults/human-api/secrets/create) — PUT a secret with full request/response.
