...
1. **A 1claw account** — Sign up at [1claw.co](https://1claw.co)
...
Claude Code uses the same MCP configuration as Cursor. Use either the **hosted** or **local (stdio)** config above (Cursor section). Then optionally [install the 1claw skill](/docs/integrations/claude-code#2-optional-install-the-1claw-skill) so Claude knows when and how to use 1claw.
