...
Same idea for chains. The private key is generated by the vault and stored envelope-encrypted
under a KMS-held KEK; the agent never sees it. It submits an intent and the vault (or Shroud's TEE)
unwraps the key, checks guardrails and signs. That is `custody: server` — see [Custody](/docs/security/custody)
for the self-custody models.
