...
**Three coding-agent compatibility bugs, all found the same day from real OpenCode traffic through `1claw proxy`.** An OpenAI-compatible client (OpenCode, via `@ai-sdk/openai-compatible`) pointed at a Claude model sends genuine OpenAI-shaped requests regardless of the model name — three places in Shroud assumed otherwise. (1) `CommandInjectionFilter` scanned the whole raw request body, so a coding agent's own system prompt/tool descriptions (which legitimately mention shell command chains) tripped `shell_command_chain`/`shell_redirect` at Critical severity and blocked every request; it now scans only message roles that can actually carry untrusted content (user turns, tool/context results), never the system prompt or the model's own replies. (2) Anthropic's Messages API requires `max_tokens`; OpenAI's does not — a request missing it was hard-rejected by Anthropic/Vertex with `max_tokens: Field required`; `ensure_anthropic_max_tokens` now fills in a default (from OpenAI's `max_completion_tokens` alias if present, else 4096) only when the field is genuinely absent. (3) A tool-use response ("Write" tool) crashed the client with `OpenAI Chat received content after the finish reason` — two independent bugs: `determine_upstream_path`'s "already versioned, keep it" shortcut forwarded a client's `/v1/chat/completions` path straight to Anthropic (which doesn't serve it) instead of translating to `/v1/messages`, and no Anthropic→OpenAI stream converter existed anywhere in the crate, so even a correctly-routed request relayed raw Anthropic SSE (`content_block_delta`, `message_delta`, …) to a client that only understands `choices[].delta`/`finish_reason`. Both fixed; the new `AnthropicToOpenAiStreamConverter` sets `finish_reason` only from `message_delta`'s `stop_reason` or `message_stop`, never `content_block_stop` (which fires after every content block, not just the last one). A related gap found while building a client-compatibility fixture harness: an OpenAI-shaped `"role":"tool"` message routed through the Anthropic parser fell through to `Unknown` and silently escaped the injection scan — `map_anthropic_role` now recognizes it too.

**Client-compatibility fixture harness.** All three bugs above shipped because the existing tests used hand-written example strings, never a real client's actual request shape. `shroud/tests/client_compatibility.rs` runs every fixture in `shroud/tests/fixtures/clients/` through the real inspection pipeline — fixtures are sourced from each client's actual public code where possible (OpenCode's real system prompt + tool descriptions from `sst/opencode`, Codex's real system prompt from `openai/codex`), clearly labeled REAL/RECONSTRUCTED/APPROXIMATED where exact source wasn't recoverable. New `shroud-check` CI job runs it (and the rest of Shroud's suite) on every PR touching `shroud/**` — nothing did before. `1claw proxy --capture-dir <path>` (CLI 0.61.20) writes every outgoing request (secrets redacted) to disk, so a future real failure can become a permanent fixture in one step instead of hand-reconstructing what the client sent.

**Two more bugs from the same investigation.** A tool-use request from the same OpenCode-targeting-Claude traffic was hard-rejected with `tools.0: Input tag 'function' found using 'type' does not match any of the expected tags`: OpenAI's function-calling tool schema (`{"type":"function","function":{name,description,parameters}}`) has no Anthropic equivalent, and OpenCode sends its bash/edit/grep/write/glob tools in that shape regardless of the target model. `convert_openai_tools_to_anthropic` now converts only genuinely OpenAI-shaped entries (`type: "function"` with a nested `function` object) to Anthropic's `custom` tool shape; an already-Anthropic-shaped tool (Claude Code, on the native SDK) passes through untouched. Separately, `reasoning_effort: Extra inputs are not permitted` — Anthropic's Messages API rejects any top-level field it doesn't recognize outright, unlike OpenAI's silent-ignore behavior; `strip_openai_only_fields_for_anthropic` now removes a denylist of 19 known-OpenAI-only fields (`reasoning_effort`, `frequency_penalty`, `presence_penalty`, `logit_bias`, `response_format`, `parallel_tool_calls`, …) before forwarding an OpenAI-shaped body to Anthropic — a denylist rather than an allowlist, so a real Anthropic field this list doesn't know about is never silently dropped. Both are covered by the client-compatibility fixture suite alongside the existing max_tokens/tools checks.

**Codex (OpenAI Responses API) targeting Claude models.** Codex's CLI always sends OpenAI's Responses API shape (`instructions`/`input[]`, no `messages` at all — current versions have no chat/completions fallback), which shares no top-level keys with chat/completions, so none of the normalization fixes above ever fired for it: a Codex request targeting a Claude model was forwarded essentially unconverted and hard-rejected. `convert_openai_responses_to_anthropic_messages` reshapes the whole body (`instructions`/developer-role items → `system`, `input[]` → `messages[]`, `function_call`/`function_call_output` → `tool_use`/`tool_result` content blocks, flat `type: "function"` tools → Anthropic's `custom` shape); a new `AnthropicToOpenAiResponsesStreamConverter` converts Anthropic SSE into real Responses API events (`response.output_text.delta`, `response.function_call_arguments.*`, `response.completed`, …) for Codex's `stream: true` traffic. A same-day follow-up fixed `determine_upstream_path`, which still forwarded a Responses-shaped, Anthropic-targeted request straight to `/v1/responses` on Anthropic's real API (which doesn't serve it) instead of translating to `/v1/messages` — the same bug shape as the chat/completions path fix above, just not extended to cover this path; caught because the original change's verification ran `cargo test --lib`, which never executes `router.rs` (a bin-only module, not included in `lib.rs`) — CI now also runs `cargo test --bin shroud`. Codex's own built-in tool types (`namespace`, `web_search`, …) and Responses-only input item types (`reasoning`, `local_shell_call`, …) have no Anthropic equivalent and are dropped rather than forwarded unconverted, flagged as a follow-up. New real fixture `codex_responses_real.json`, captured via `1claw proxy --capture-dir` from an actual `codex exec` run.

**Examples reorganized.** The four separate `shroud-demo`/`shroud-llm`/`shroud-router-key`/`shroud-security` example projects are now one `examples/shroud/` with four numbered sub-examples (`01-basic-demo`, `02-llm-billing`, `03-router-key`, `04-security-offline`) and a README explaining which to pick; doc links updated to match. Three new examples: **Browser Bridge** (`@1claw/browser-bridge` — the agent drives the page with real `puppeteer-core`, a separate process types the password, the agent never receives or reads the value), **Telegram Channel** (register a bot, send/list messages through the Channels API), and **Passkey Safe** (a Gnosis Safe whose only owner is a WebAuthn passkey via Safe's `SafeWebAuthnSharedSigner` — no private key exists anywhere for the Safe).

**Guided onboarding: coding agents get the whole flow, not just MCP.** The coding-agent path now also walks through connecting Codex, OpenCode, OpenClaude, or Claude Code to Shroud/LLM Token Billing via the local `1claw proxy` — one command, both MCP and LLM routing configured. `1claw setup` gained real Codex/OpenCode MCP auto-configuration (`~/.codex/config.toml` `[mcp_servers.1claw]`, `~/.config/opencode/opencode.json` `mcp.1claw`) instead of only listing traditional GUI clients, plus install hints (e.g. `npm install -g @openai/codex`) next to any client it didn't detect. Fixed a same-tab Stripe-checkout redirect that lost in-progress wizard/chat state on several pages (checkout now opens in a new tab); fixed a race that sent onboarding-eligible users to the wrong (developer) onboarding path instead of the guided one; reworked the OIDC Federation (Anthropic WIF) setup UX to auto-fill the audience and walk through registration step by step.

**Platform-default web search.** Agents with no `BRAVE_API_KEY`/`TAVILY_API_KEY`/`SERP_API_KEY`/binding of their own now get a working `web_search` tool by default: `POST /v1/tools/web-search`, proxied server-side through a shared 1claw-provided Brave Search key (never reaches a tenant runtime), hard-capped at 900 requests/month platform-wide (`platform_web_search_usage`, comfortably under Brave's free-credit volume).

**Adoption/usage metrics, one source for the dashboard and the investor report.** New `GET /v1/admin/stats/growth-timeseries?vertical=&period=` (15 verticals — signups, subscriptions, Shroud usage/security, execution/transaction intents, vault/runtime/memory usage, automations, embedded wallets, platform API — gap-filled 24h-hourly/7d-daily/30d-daily, with a period-over-period delta) and `GET /v1/admin/stats/retention?window_days=` (% of orgs older than the window still active). Wired into the live admin analytics dashboard (`/settings/admin/analytics`, a new Adoption & Usage section plus a retention tile) and a rewritten weekly investor report: the email body is now a curated summary (a 24h/7d/30d table across all verticals, real charts for the four highest-signal ones, a retention card, a cumulative-growth curve), full per-vertical/per-window detail moves to the PDF attachment. Charts are rendered locally as SVG (`scripts/lib/svg-charts.mjs`) instead of through a third-party charting service, which previously carried real MRR/revenue figures in a public URL. `weekly-investor-report.yml` gained a `dry_run` input that renders against real data without emailing or writing the trend-snapshot row. Also fixed a long-silent bug: `/v1/admin/stats`'s Memory stats queried a table (`agent_memory`) that never existed — the real table is `agent_memory_entries` — so those four fields always silently read zero.

**Live-tested every genuine coding-agent CLI against `1claw proxy`.** Claude Code, Codex, OpenCode, OpenClaude (`@gitlawb/openclaude`), and Goose (Block's agent) all confirmed clean — Claude Code/OpenClaude/Goose needed no fixes, sending either native Anthropic or the same OpenAI-shaped traffic the fixes above already cover. Gemini CLI (`@google/gemini-cli`) is different: it speaks Google's own native `contents`/`systemInstruction` format to `/v1beta/models/{model}:generateContent`, not OpenAI's. The proxy's provider auto-detection had no case for that path and defaulted to `openai`, so Shroud couldn't parse `req.messages` from the mismatched body and fell back to a raw-full-body scan — false-positive-blocking on ordinary shell syntax in Gemini CLI's own system instructions, the same false-positive class as bug (1) above, here caused by a provider mislabel rather than a scan-scope bug. **`@1claw/cli` 0.61.21** now recognizes `/v1beta/models/{model}:(generateContent|streamGenerateContent|countTokens)` and routes it as `provider: google`, mirroring the existing `/v1/messages` → `anthropic` special case. New real fixtures `openclaude_anthropic_real.json` and `goose_anthropic_real.json` in the client-compatibility suite; see [IDE & tool setup](/docs/agents/shroud/ide-setup) for per-client instructions.

**Audit remediation.** STRIPEPATH-C1 (a client-supplied `setup_intent_id` was path-injected into a Stripe API call made with the platform's live secret key), SETUPINTENT-H1 (the trial gate never checked the SetupIntent belonged to the caller's org), AIBUDGET-H1 (daily AI spend was a non-atomic check-then-record; now an atomic reserve/settle against `agent_daily_ai_spend`/`agent_daily_ai_spend_holds`), AIMODEL-H1 (the client-supplied `X-Shroud-Model`/`-Provider` headers could diverge from the body actually billed; the body now wins), TXBUDGET-RACE (the same class of race in value-based daily transaction budgets, closed by widening the existing daily-tx-cap advisory lock), AUTOPARK-M1 (a nested `execute_intent` step inside a parked automation was allowed through), AUTORATE-M1 (automation-triggered binding executions skipped the rate/concurrency guardrails the direct execute path enforces), RUNTIMEROLLBACK-M1 (a runtime restart proceeded even when stopping the old container failed), CHANAUTORESPOND-M1, PAYPAUSE-M1.

### Dashboard 2026-09-22 {#dashboard-2026-09-22}

**Guided onboarding.** A SetupIntent-gated free trial (card required, not charged unless kept) alongside the existing subscribe-to-Pro path, an AI spend/pricing/budget engine, the first version of the weekly investor report (adoption/usage/revenue over the trailing 30 days, HTML email + PDF, Stripe MRR pulled live), and a large dashboard UX restructure. Migs 281–283.

### Dashboard 2026-09-21 (b) {#dashboard-2026-09-21-b}

**One way to do each thing.** `/agents/new` and `/vaults/new` redirect to the wizards (the agent wizard's Advanced step now carries auth method with mTLS/OIDC fields, scopes, token TTL, expiry, environment and memory; sub-agents are linked from the wizard rather than offered as a second button); the runtime list's Quick Create is gone; every list and empty state has one primary button. **One Inbox** (`/inbox`: Agent approvals · Team consensus · Wallet proposals, one badge) replaces three sidebar entries; the old routes redirect. **Settings** are grouped (You · Organization · Security · Developers · Billing) with a search box; **Billing** is Plan / Usage / Payments / Add-ons; **Security** is Sign-in / Data protection / Guardrails / Honeytokens with org-wide policies under Advanced. **Forms:** the access-policy form is three fields in Simple mode with a plain-English preview of the policy it will create (effect, priority, conditions, consensus under Advanced); the secret form is path + value (type, tags, description, expiry, access cap under Advanced). **Pages:** platform app tabs get Simple (Overview · Users · Templates + More), the vault page renames Policies to *Access*, explains env vars, and keeps encryption settings under Advanced; Treasury has a first-run explainer. **Wayfinding:** sidebar says Agents / Runtimes; a persistent setup checklist on the Overview; one status pill style across lists.

### v0.61.49 (2026-09-21) {#v06149-2026-09-21}

**AI spend.** Every LLM request through Shroud is now **priced at ingest** from a per-model price card (mig 281: `shroud_activity.cost_micro_usd` + `priced`; `llm_model_prices` seeded with September 2026 list prices for Anthropic, OpenAI, Google/Gemini, Mistral, DeepSeek, xAI, Groq, Together, OpenRouter, Venice, Ollama; existing rows backfilled — 8,013 of 8,016 in prod). New `GET /v1/spend/ai?from&to&interval&agent_id&provider`: totals with the previous window, a series by provider, breakdowns by provider / model / agent (each agent with its `daily_budget_usd`, today's spend, utilisation and requests blocked for budget), router-key spend caps, credit balance and 1Claw inspection fees; `GET /v1/spend/ai/export.csv`; the price card at `GET/PUT /v1/spend/ai/prices`, `DELETE /v1/spend/ai/prices/{id}`, `POST /v1/spend/ai/reprice` (owner/admin). The dashboard control plane gains a **Spend** tab (`9`) — Ramp-style: period/provider/breakdown/compare filters, KPI cards with deltas, stacked spend-over-time, provider donut, top models, agent table with budget bars, limits panel, price editor, CSV export. **OTEL:** each request also emits `ai.cost_usd`, `ai.tokens.input`, `ai.tokens.output` (attributes `agent.id`, `ai.provider`, `ai.model`, `ai.action`) so an org's own collector gets the same numbers. The system overview's inference spend now reads the priced rows (the ledger it read before was never written). **Shroud enforces daily budgets from the same card** (Shroud fetches `GET /v1/admin/llm-prices` every 5 min, prices each request identically, and re-syncs each agent's UTC-day counter from `GET /v1/shroud/activity/today` every minute — the old flat $0.01/1K in-memory counter that reset 24 h after first use and was lost on restart is gone); budget blocks are 429 with the spend named; allowed responses carry `x-shroud-budget-limit-usd` / `-spent-usd` / `-remaining-usd`. `@1claw/sdk` 0.61.33 (`spend.ai/listPrices/setPrice/deletePrice/reprice`), CLI 0.61.19 (`1claw spend ai|export|prices`). Spec 0.61.40.
...
### v0.61.46 (2026-09-20) {#v06146-2026-09-20}

**Dashboard: one overview of the whole system.** New `GET /v1/org/overview?hours=` returns inventory, activity in the window, spend, health signals and a 7-day trend in one call (every subquery org-scoped; guard test). The dashboard control plane gains an **Overview** tab (needs-attention list, activity, inventory, spend, trend — each number links to where you act), and the former *Activity* pages are now tabs on the same screen (**Shroud**, **Risk**, **Audit**) with Topology still first; `/platform-activity/*` redirects, `?view=` deep-links, keys `1`–`8`. `@1claw/sdk` 0.61.31 (`org.overview()`), spec 0.61.37.

**Automations UX (uplift batch B).** **Dry run:** `POST /v1/automations/{id}/dry-run` and `POST /v1/automations/dry-run` (unsaved spec) render every step after template substitution — resolved fields, side effect, `on_error` policy, templates that resolve to nothing, conditions, the budget crossing — with nothing done; **Preview run** in the editor and on the detail page. **Run timeline:** step results record `duration_ms`, `attempts`, `continued` and the resolved `input` (credentials removed); the dashboard run dialog is a timeline with expandable input/output, attempts and cost, a parked-run banner with inline Approve/Reject (or "skip the wait" / "continue without the callback" for owners), and **Re-run from here** on every step (`POST …/runs/{run_id}/rerun {from_step}` — a new run seeded with the finished run's results). **Versions panel** on the detail page with one-click rollback. The editor knows the v2 step types (`wait_until`, `awaiting_callback`, `for_each`, `call_automation`) and has **Step name** and **On error** (fail / continue / retry ×N with backoff) on every step. Fixed: the dashboard read run fields the API never returned (`trigger`, `completed_at`, `step_type`), so the Runs table showed blank triggers and a step card threw on open — runs are now normalised in one place. `@1claw/sdk` 0.61.30 (`dryRun`, `rerunFromStep`), spec 0.61.36.

### v0.61.45 (2026-09-20) {#v06145-2026-09-20}

**Automation engine v2 (runtime & automations uplift, batch A).** Every step may carry an `on_error` policy — `fail`, `continue`, or `retry` with bounded linear backoff (fund-moving and parking steps are never retried). Specs may carry a `budget` (`max_tokens`, `max_cost_cents`, `max_steps`, the last counting every loop iteration and sub-workflow step). Two new parks stop the 300-second clock like `approval_request` does: **`wait_until`** (the scheduler wakes it, up to 72 h) and **`awaiting_callback`** (the run gets a per-run token at start as `{{run.callback_url}}`; `POST /v1/automations/{id}/runs/{run_id}/callback/{token}` continues it with the body as `{{resume.*}}`). New control flow: **`for_each`** (bounded fan-out with `{{item}}`/`{{index}}`) and **`call_automation`** (inline sub-workflow, depth 1, recorded as its own run with `parent_run_id`). **Agent-created automations may now use `execute_intent`** — network access only through a binding a human created, with the binding's allowlists, conditions, secret scan and approval policy applied in the executor exactly as on the direct execute path; a policy that wants a human parks the run and re-runs the step once approved. **Versions:** every `workflow_spec` is kept (mig 277 `automation_versions`; `GET …/versions`, `POST …/versions/{n}/rollback`); runs record `spec_version`. **Widening edits** (adding fund-moving or network-reaching steps, raising a budget cap) go through control-plane consensus under the new `automation.widen` action; `PATCH` now validates step types like `POST` always did. **Idempotent triggers:** `{input, idempotency_key}` / `Idempotency-Key` on trigger and webhook (GitHub delivery ids and Stripe `evt_` ids are used automatically) — the same key returns the existing run with 200. **Signed inbound webhooks:** `webhook_signature` `{scheme: stripe|github|hmac_sha256, secret, header?, tolerance_secs?}` on a webhook automation; the secret lives encrypted in the agent-keys vault and an unsigned or mis-signed delivery starts nothing. Run responses carry `paused_reason`, `paused_until`, `paused_expires_at`, `spec_version`, `idempotency_key`, `parent_run_id`; new run statuses `waiting` and `awaiting_callback`. `@1claw/sdk` 0.61.29 (`trigger(id, input, {idempotencyKey})`, `listVersions`, `rollback`, `callback`, `StepOnError`, `RunBudget`), CLI 0.61.17 (`automation trigger --input --idempotency-key`, `versions`, `rollback`), MCP 0.63.5 (`trigger_automation` takes `idempotency_key`; the SDK/MCP trigger body previously sent a bare object the vault ignored). Spec 0.61.35.

### v0.61.44 (2026-09-20) {#v06144-2026-09-20}

**Security audit remediation (2026-09-20 cycle: 8 HIGH, 8 MEDIUM, 4 LOW).** Router-key tokens are routing-only: the vault refuses every route outside the gateway surface (inspection debit, vault list, provider/api-key reads, agent self, bankr leases, activity, health) and Shroud refuses execution/transaction paths for them. Passkey-Safe agent spends run the org's signing policies and the consensus gate like every other send, and both relayer-key fetches go through one wallet-access-gated, audited primitive. `POST /v1/automations/{id}/runs/{run_id}/resume` no longer decides the approval — a pending one is 409 with a pointer to `POST /v1/approvals/{id}/decide` — and a parked run pins its `workflow_spec` (hash + copy) and fails rather than run an edited workflow under an old approval. Tool bindings require the agent (or the creating user) to already read the secret. Deactivating a parent agent stops its children's inherited policies. Pairing approve/deny is compare-and-set (409 on replay). Included runtime slots are claimed under a per-org lock. K_enclave release fails closed if its audit row cannot be written, pins RS256, and refuses `secboot=false` / non-confidential `hwmodel`. Recovery codes are spent on redemption (a fresh one is shown). Shroud: the client IP is the TCP peer, with `X-Forwarded-For` believed only behind a trusted proxy (`SHROUD_TRUSTED_PROXY_CIDRS`), and a per-IP pre-auth rate limiter (`SHROUD_PREAUTH_RATE_LIMIT_PER_MIN`, default 600) sheds unauthenticated floods; `deploy-shroud.yml` attaches Cloud Armor network DDoS protection to the NLB and refuses to retire the L7 Ingress until it is verified. CI: `docker-build` and `deploy-vault.yml` both gate on the migrations applying to a fresh Postgres. Dashboard: Settings → Connected Apps gains **Connect Muse**.

### v0.61.43 (2026-09-20) {#v06143-2026-09-20}

**Router-rail fixes found in production, and the site catches up.** The inspection-fee debit now reaches the vault: Shroud calls it over `SHROUD_VAULT_INTERNAL_URL` (the public API host proxies only `/v1/*`) and the route lives in the authenticated group rather than the Intents-gated one (a router-key agent need not have the Intents API on) — before this every router-key `POST` failed closed. The 1 MB router-rail cap runs right after authentication, ahead of provider-key resolution (413, never a 401 about a missing key). `POST /v1/x402/topup` claims the payment proof before looking the wallet up, so a replay never opens a shadow org. Shroud rollouts that stall on node-pool capacity with ≥ 2 new pods ready are treated as deployed; the HPA no longer scales on memory. Docs: API reference gains child agents, router keys, tool bindings, connectors/event subscriptions, charts and the x402 top-up; `/for-ai` gains four Shroud cards; `1claw.co/shroud` gains the drop-in section; the billing page explains what credits pay for. New example `examples/shroud-router-key`. `@1claw/sdk` 0.61.28, `oneclaw` 0.61.13 (READMEs). Ops runbook `internal-docs/runbooks/shroud-ai-router-rail.md`.

### v0.61.42 (2026-09-20) {#v06142-2026-09-20}

**Format adapters, the offline conformance gate, Signal T0, the egress lock.** Shroud's canonical IR now covers the OpenAI **Responses API** (`instructions` + `input` items, `function_call` / `function_call_output`; streamed `response.output_text.delta` inspected per frame and `response.function_call_arguments.*` assembled like chat tool calls) and **MCP JSON-RPC** (`tools/call`, `sampling/createMessage`). `tests/conformance_track_a.rs` is the offline half of the conformance suite — 102 vectors from `tests/conformance/config.toml`'s bars (single-arg, DSN-embedded, streaming-split at every third byte of every secret, prompt-cache stability, and the adversarial set: unauthorised tool, exfil host, wrong argument, markdown-image exfil, amplification, bad checksum, legacy tag) — currently leak rate 0, rehydration 1.0/1.0, adversarial refused 1.0. Blocked-request audits carry **Signal T0 fingerprints** (SHA-256 + 64-bit SimHash over 3-word shingles of the redacted, placeholder-stripped span; hashes only, no verdict changes). `k8s/networkpolicy-egress-fqdn.yaml` names the vault, providers, KMS, ACME, facilitator and RPC hosts and drops the `0.0.0.0/0:443` rule when `SHROUD_EGRESS_LOCK=true` (needs GKE FQDN network policies). `@1claw/sdk` 0.61.27 / `oneclaw` 0.61.12 / CLI 0.61.16: tool bindings (`createToolBinding`…, `agent bind-secret | tool-bindings | unbind-secret`). Spec 0.61.34.

### v0.61.41 (2026-09-19) {#v06141-2026-09-19}

**Placeholders the model can place but never read; rehydration only inside the enclave.** With the attestation-released enclave key, Shroud now replaces vault secrets in prompts with `⟦sk:{type}:{tag}{ck}⟧` placeholders — deterministic per (org, secret), so prompt caches survive; rotated with the secret; checksummed; typed (`pg-dsn`, `bearer`, `pem`, …) — and keeps a per-org reverse index in enclave memory (legacy `[REDACTED:#…]` stays when no key is held). `POST /v1/agents/{agent_id}/tool-bindings` (`GET`, `DELETE …/{binding_id}`; human-only; migration 276 `secret_tool_bindings`, RLS) records which secret may be rehydrated for which agent, tool and argument (JSON pointer) and toward which destination hosts. The vault forwards an agent's bindings with every TEE execution; the enclave substitutes within strings only where a binding matches the resolved destination host, refuses (403) unbound arguments, wrong hosts, bad checksums, unknown tags and >4 repeats, and reports the secret paths it used, never the values. A non-overridable egress deny-list (cluster-local services, the vault and gateway hosts, `ONECLAW_EGRESS_DENIED_HOSTS`) runs before any customer allowlist. Spec 0.61.33.

### v0.61.40 (2026-09-19) {#v06140-2026-09-19}

**Router rail is paid from request one; the ledger is micro-USD; the x402 agent door opens.** Migration 274 moves the prepaid ledger to micro-USD (`organizations.credit_balance_micro_usd`, `credit_transactions.*_micro_usd`; the cents columns every reader used become derived, so nothing that read them changes) and `GET /v1/billing/credits/balance` / the subscription response now carry `balance_micro_usd` and dollars at the ledger's precision. Shroud debits **$0.005** per inspected router-key request at accept (`POST /internal/v1/shroud/inspection-debit`: idempotent on the request id, FIFO across top-ups, counted against the key's `spend_cap_usd`, TEE-origin gated); an in-flight stream is never cut off for billing — the next request gets 402. Router-rail bodies cap at 1 MB (413). `POST /v1/x402/topup` is the wallet rail: a 402 quote for $1 of USDC on Base (up to $100), then verify → claim → credit → settle; an unknown wallet's first settled top-up provisions a shadow org + Shroud agent + router key (returned once) after OFAC SDN screening (fails closed) and the geofence — migration 275 `x402_wallet_orgs`, same wallet = same org. The gateway drives it for `Authorization: Bearer x402`: no `X-PAYMENT` → the quote; with one → the request is served on a ≤60 s token and answers carry `x-shroud-router-key` (first time) and `x-shroud-balance-micro-usd`. Metrics `shroud_inspection_fees`, `shroud_inspection_fee_refusals`, `shroud_x402_challenges`, `shroud_x402_topups`. Spec 0.61.32.

### v0.61.39 (2026-09-19) {#v06139-2026-09-19}

**Router keys: a stock SDK can talk to Shroud.** `POST /v1/agents/{agent_id}/router-keys` mints an `sk-shroud-v1-<32>` key (plaintext returned once, with `base_url`), `GET` lists, `DELETE …/{key_id}` revokes — human-only, Shroud-enabled agents, 20 live keys per agent, optional per-key `max_concurrent_streams` and `spend_cap_usd`. The gateway now accepts a lone `Authorization: Bearer` in any of four forms — router key, lone `ocv_` agent key, `agent_id:ocv_`, or an agent JWT — and exchanges the key-only forms itself (`POST /v1/auth/agent-token` with `api_key` alone). Router-key tokens carry a `router_key` claim and live at most 60 s, so a revoke lands at the gateway inside the SLA; open streams are counted per router key. `Authorization: Bearer x402` is answered with a 402 challenge (`x402Version: 1`, `accepts[]`) instead of a 401, so a paying client can settle and retry once the ledger path lands. Migration 273 (`agent_router_keys`, RLS). Spec 0.61.31. New prod suite `scripts/test-router-keys-prod.sh`.

### v0.61.38 (2026-09-19) {#v06138-2026-09-19}

**Shroud streams are inspected per frame.** `stream: true` through the Shroud proxy no longer buffers the whole response (and never forwards it raw): text deltas are released behind a tail buffer sized to the org's longest vault secret (≤ 256 bytes; longer values matched by prefix and swallowed), so a secret split across chunks is redacted before any byte of it reaches the client; tool-call deltas (OpenAI `delta.tool_calls[]`, Anthropic `input_json_delta`) are held until the call is complete, checked against the agent's tool policy, then released in their original frames and order. Two modes on `shroud_config`: `streaming_inspection_mode: "rolling"` (default; injection heuristics warn-only, blocks only on secret literals, denied tools, secret-bearing tool arguments or an exfil URL beside a redaction) and `"holdback"` (`streaming_holdback_chars`, default 512; injection blocks as on the buffered path). A mid-stream block sends one error frame and closes cleanly; OpenAI-format requests gain `stream_options.include_usage` so token counts still land; Shroud Activity rows are finalised at stream end; Gemini stays buffered. Per-key cap of 20 open streams (`max_concurrent_streams`), `429` + `Retry-After` beyond it. New metrics `shroud_stream_frames`, `shroud_stream_frame_inspection_seconds`, `shroud_streams_blocked`, `shroud_streams_over_concurrency`. OpenAPI spec 0.61.30 (the three new `ShroudConfig` fields).
...
### v0.61.31 (2026-09-19) {#v06131-2026-09-19}

**Charts create policies, connectors and bindings** (nanobots plan, item 8). `1claw apply` used to validate a chart's `policies` and `connectors` and then create only the vaults and agents; both are now created through the same gated handlers a dashboard click takes, in dependency order (vault → agent → policy → connector → binding). A connector install returns its `authorization_url` on the resource — OAuth consent is the only human step — and a re-apply after the sign-in reports it `unchanged`. New `spec.agents[].bindings`: an execution-intent binding declared in full (`binding_type`, `config`, `guardrails`), with two rules the API alone does not impose — `guardrails.allowed_hosts` must be non-empty (the binding's egress is the declaration) and the credential is a pointer into a chart vault (`credential: {vault_ref, path}`, a `vault_ref` credential source), never an inline value. Policies are keyed per path (`vault:agent:path`); permissions, binding config/guardrails, connector presets and vault fields are create-only and a difference is `refused` naming the endpoint that owns it. An agent's `description` / `system_prompt` are now patched in place through `update_agent` (result `patched`). A dependent whose vault or agent was queued for approval this run is `skipped` as "waiting on …" and created by the next apply. `scripts/test-chart-apply-prod.sh` section 5. **Deploy fix:** migration 269 wrote a JSON-style literal into a `TEXT[]` column and failed the Cloud Run startup probe (production stayed on the previous revision); fixed, and CI now applies every migration to an empty pgvector Postgres so SQL that never runs under `cargo test` is exercised before a deploy. OpenAPI spec 0.61.27, `@1claw/sdk` 0.61.21, CLI 0.61.12.

### v0.61.30 (2026-09-19) {#v06130-2026-09-19}

**`binary` runtime template** — run a compiled program (Go, Rust, …) from a release asset (`env_public.BINARY_URL`, HTTPS, pinned by `BINARY_SHA256`, archives unpacked) or a `startup_command` after a repo clone; the create call refuses a binary runtime with nothing to run. CLI 0.61.11 `runtime create` lists the real templates/presets and gains `--source-repo`, `--source-branch`, `--startup-command`. **Platform-default OAuth apps** — `ONECLAW_OAUTH_DEFAULT_APPS` (JSON, per provider) lets connections use a 1Claw-owned app when the org registered none; org credentials always win; the dashboard shows "Using 1Claw default"; `GET …/oauth/app-credentials` returns `platform_defaults`. Registering the apps and Google's restricted-scope verification remain the operator's decision. **Automation runs resume after their approval.** An `approval_request` step already parked the run and stopped the clock; nothing woke it up (nanobots plan, item 5). Now the run's state is saved at the step (migration 270), deciding the approval resumes it automatically (approved → next step, rejected → failed with the reason), `POST /v1/automations/{id}/runs/{run_id}/resume` hands off a decision made elsewhere (human-only, optional `payload` as `{{resume.*}}`), the resumed segment gets its own 300-second clock, and a run parked for more than 72 hours becomes `timed_out`. New webhook event `automation.run.resumed`. SDK `automations.resumeRun`, Python `resume_run`. **Child agents** (nanobots plan, item 3) — `POST /v1/agents/{agent_id}/children` creates a sub-agent under a parent (human-only, up to 50 per parent, migration 271): it gets its own API key, memory namespaces (default `child:{id}`) and action-approval policy; its vault_ids and scopes must be a subset of the parent's (a superset is refused); it inherits the parent's vault policies and guardrails; it does not count against the plan's agent cap, cannot have children of its own, and reports `agent_type: "child"` / `parent_agent_id` on every agent response. `GET …/children` lists them. SDK `agents.createChild` / `listChildren`, Python `create_child` / `list_children`. OpenAPI spec 0.61.26, `@1claw/sdk` 0.61.20, `oneclaw` 0.61.7.

### v0.61.29 (2026-09-19) {#v06129-2026-09-19}

**Connector catalogue: Google Drive, Google Business Profile, Stripe, HubSpot, LinkedIn, and a generic `api-token` connector** (host named at install, token stored in the vault, same SSRF rules as every binding; migration 269 seeds the three new OAuth providers and the Google `drive.file` / `business.manage` scopes). **Three things nanobots asked for.** `POST /v1/agents/{id}/memory/search` now matches text stored in *values* — it decrypts the namespace (bounded, most recent 500 + shared) and scores key + value lexically (exact 1.0, substring 0.95, token overlap 0.3–0.9); it returned nothing for any value text before. `POST /v1/approvals/{id}/cancel` lets the requesting agent (or the addressed human) withdraw a pending approval, first-answer-wins, delivered to subscribers as `approval.decided` with `decision: "cancelled"` (new status `cancelled`, migration 268). `GET /v1/approvals` as an agent returns the approvals that agent created. SDK `approvals.cancel`, Python `approvals.cancel`, MCP `cancel_approval` (157 tools). The memory prod suite now asserts a value-text hit instead of accepting zero results.

### v0.61.28 (2026-09-19) {#v06128-2026-09-19}

**One Agent Runtime included with every paid plan.** Pro, Team, Business and Enterprise each include one Cloud Runtime at no charge — Small or Medium (up to 1 vCPU / 2 GB), with no monthly hour cap and no Stripe add-on. The vault assigns the included slot to the first small/medium runtime you create (`billing_kind: included`; `GET /v1/runtimes` reports `included_runtime: { allowance, used, presets }`), the create wizard shows "Included with your plan" instead of a price, and deleting the runtime frees the slot. Large and Confidential Compute presets, and further small/medium runtimes, remain paid add-ons; pool runtimes still draw from the plan's hours. Medium is now 1 vCPU / 2 GB (was 1 GB). Migration 267. OpenAPI spec 0.61.25, `@1claw/sdk` 0.61.19. **Also:** passkey-Safe `execute` checks the relayer's gas balance before signing and names the address, chain and amount to fund; the Safe card shows the gas payer. Pairing approve/deny now read the pairing columns (the previous column list dropped them, so `--pair` never received its key) — caught by the new `scripts/test-custody-prod.sh` on its first run.

### v0.61.27 (2026-09-19) {#v06127-2026-09-19}

**Passkeys registered on the old domain no longer lock an account.** A credential is bound to the rpId it was registered under, so a `1claw.xyz` passkey cannot answer on `1claw.co`; every step-up gate now counts only passkeys usable on the current domain — registering a new passkey no longer demands a step-up nothing can satisfy, and a stranded credential on an account with no other factor can be removed with the live session (audited). The security page marks such passkeys "Only works on 1claw.xyz". **Custody deny switch at the insert:** `ONECLAW_SERVER_CUSTODY_GENERATION=deny` now covers signup auto-provisioning, platform bootstrap and the smart-account signer, because the check lives where every server-custody key is inserted. **Agent TSS parity:** agent-driven threshold sends honour per-agent wallet-access policies (and so do delegated server-custody key reads); a background sweeper reaps abandoned ceremonies' round secrets; a failed ledger write after broadcast is an error and an audit event, not silence; runtime-share provisioning refuses platform-claim sessions. Admin usage-pattern reads are audited. PostHog identify no longer sends platform-admin status. Runtime negative tests for forged passkey assertions and server-share-only signing. New prod suite `scripts/test-custody-prod.sh`.

### v0.61.26 (2026-09-19) {#v06126-2026-09-19}

**Agents spend from passkey-owned Safes, capped on chain.** *Agent spending* on a passkey Safe grants an agent a per-period allowance through Safe's Allowance Module v1.0.0: one passkey touch enables the module, adds the agent's Ethereum signing key as a delegate and sets the cap (`POST /v1/treasury/passkey-safes/{id}/grants`, `…/grants/{grant_id}/revoke`; `execute` accepts a delegatecall only into the pinned MultiSendCallOnly and only for calldata it prepared). The agent spends with `POST /v1/agents/{agent_id}/passkey-safes/{safe_id}/spend` (`agents.spendFromPasskeySafe` in `@1claw/sdk` 0.61.17, `spend_from_passkey_safe` in `oneclaw` 0.61.4) after its guardrails and the sanctions screen; the module — not the vault — enforces the cap. Counts as one signature. **Passkey Safes name their passkey**, `prepare` returns its `transports` (Touch ID instead of the cross-device QR), and a passkey registered on another domain (`1claw.xyz` vs `1claw.co`) is refused at create/prepare with the reason instead of a prompt that can never complete. **Canonical domain sweep**: every remaining `1claw.xyz` default, link and message in the vault, packages, scripts and internal runbooks now says `1claw.co` (`.xyz` stays accepted as legacy); runtime slug URLs mint `{slug}.run.1claw.co`. OpenAPI spec 0.61.24.
...
### v0.61.23 (2026-09-18) {#v06123-2026-09-18}

**Agents sign with self-custody keys, unattended.** The second share of a `client_tss` wallet can be held by the Shroud sidecar in a hosted runtime: the sidecar registers a P-256 key at boot (`POST /v1/runtimes/{id}/tss/holder`), the owner's passkey re-wraps their share to it (`PUT /v1/keys/{id}/client-share/holder`, ECIES, opaque to the vault), and the agent sends through `/v1/agents/{id}/tss/{prepare,sign/begin,sign/complete,broadcast}` — `prepare` applies the agent's guardrails and the sanctions screen and records the message, `sign/begin` refuses anything unprepared, `broadcast` verifies and records the transaction. Sidecar 1.1.0 runs the same FROST crate as the browser via wazero (`POST /tss/send` on its loopback API). **Re-key to self-custody**: a server-custody Solana wallet can be replaced by a threshold key in one ceremony (`replace_existing`), the old key's last signature sweeping its balance across (audit `treasury_wallet.rekeyed`). **Custody policy switches**: `ONECLAW_SERVER_CUSTODY_GENERATION=deny` (with an exception list) and `ONECLAW_LAZY_WALLET_PROVISIONING`. **Proof**: `no_server_only_reconstruction.rs` fails the build for any key read that is not gated on custody, threshold, or passkey-owner — and found two ungated server paths (card orders, x402 pay) which are now gated. OpenAPI spec 0.61.21.

### v0.61.22 (2026-09-18) {#v06122-2026-09-18}

**Passkey-owned Safes on EVM (`custody: passkey_owner`).** A Safe v1.4.1 whose only owner is Safe's WebAuthn shared signer configured with your passkey's P-256 key — no private key exists anywhere. `POST /v1/treasury/passkey-safes` derives the counterfactual address; `…/{id}/prepare` runs the sanctions screen and spend policies and returns the SafeTx hash, which your passkey signs as its WebAuthn challenge; `…/{id}/execute` verifies the assertion locally (this passkey, this hash, a 1Claw origin, user verification) and relays `execTransaction` — deploying the Safe on the first send — from your Ethereum treasury wallet, which pays gas and is not an owner. Base, Optimism, Arbitrum and Polygon verify through the RIP-7212 precompile; Ethereum and the Sepolias through the Daimo P-256 verifier. Treasury → *Passkey-owned Safes* in the dashboard. **Safe registry corrected:** the pinned v1.4.1 singleton and proxy creation code were not the canonical ones (the singleton had no code on any chain) and the SafeTx struct hash double-hashed `data`; counterfactual addresses now match `SafeProxyFactory.createProxyWithNonce` on Base byte for byte, and the Allowance Module pin is v1.0.0. No existing account was affected (no counterfactual agent Safes had been created). OpenAPI spec 0.61.20.

### v0.61.21 (2026-09-18) {#v06121-2026-09-18}

**Self-custody Solana wallets (`custody: client_tss`).** A 2-of-2 FROST/Ed25519 key generated between the vault and your browser (`@1claw/tss-wasm`); the private key never exists. Your share is wrapped under your passkey's PRF output and stored opaque; the vault's is a `tss_share` secret no server signing path reads. New: `POST /v1/keys/tss/keygen/{begin,round2,complete}`, `POST /v1/keys/{id}/tss/sign/{begin,complete}` (passkey-bound to the message digest; the vault decodes the message and runs the sanctions screen on every transfer inside it; nonces are single-use), `POST /v1/treasury/wallets/{chain}/tss/{prepare,broadcast}` (wallet access, sanctions, spend policies; broadcast only a verified signature over the screened message). Treasury → *Self-custody Solana wallet* in the dashboard; tx-assert actions `tss_keygen` / `tss_sign`. **Server-side product analytics**: the vault reports named API actions (token exchange, secret read/write, sign/submit, approvals, runtime chat/shell, treasury sends) to PostHog under the same identity as the dashboard, grouped by org. OpenAPI spec 0.61.19. EVM/Bitcoin/Tron keys remain `server` custody; see [Key custody](/docs/security/custody).

### v0.61.20 (2026-09-18) {#v06120-2026-09-18}

**Sanctions screen on every signing path.** Agent submit/sign, the unified `/sign`, automations, the TEE-forwarded path, human treasury sends and Safe proposals (creation and execution) now check the destination — and the recipient inside ERC-20 calldata — against the OFAC SDN digital-currency list before anything is signed. A hit is refused with `403` naming OFAC and audited as `sanctions.blocked`; no allowlist, cap or policy admits it. The list is refreshed daily from Treasury's SDN publication and the screen fails closed: older than seven days and transfers pause with `503` until it refreshes. **Custody is now a fact on the key.** `custody` (`server` today; `client_tss` for passkey-held threshold keys) is on every treasury wallet and agent signing key, the dashboard shows a Custodial / Self-custody badge, and the server signing path refuses to unwrap a `client_tss` key — see [Key custody](/docs/security/custody). **Client-held shares:** `PUT`/`GET /v1/keys/{key_id}/client-share`, `POST …/client-share/rewrap` and `DELETE …/client-share/{share_id}` store the customer's wrapped share of a threshold key, opaque to the vault. **Passkeys report PRF support** (`prf_supported` on registration and in the list; "Can hold wallet keys" in Settings → Security). **Geofence:** creating a 1Claw-held wallet is refused from New York (`403`, audit `custody.geofenced`) until threshold keys ship; signup from New York proceeds without the auto-provisioned wallet. **TEE key delivery is no longer metered** (`GET /internal/v1/keys/…` was billed as a customer request, so TEE signing 402'd for an org in overage and every TEE sign cost two metered calls) and reports the key's real `key_version` on explicit paths. OpenAPI spec 0.61.18.

### v0.61.19 (2026-09-17) {#v06119-2026-09-17}

**`intents_require_tee` no longer needs client-side routing.** A transaction submit or sign that reaches `api.1claw.co` for a TEE-required agent is carried to Shroud by the vault (the agent's own JWT is replayed; Shroud signs, broadcasts and records as for a direct call). MCP, SDK and CLI clients that only know the vault host now work with the flag on; `execution_require_tee` likewise forces the TEE execution path instead of refusing. Refusals remain for delegated/human credentials and DPoP-bound tokens. **Failed broadcasts release their nonce.** A submit whose RPC broadcast fails now gives the nonce back, and the nonce tracker only stays ahead of the chain's pending count when the gap is covered by the vault's own recent broadcasts — a failed broadcast can no longer leave the next transaction queued behind a hole. **Approval emails say what is being approved:** reason, target and the request's summary fields are rendered as rows alongside the action.
...
Four features shipped in this release. Peer memory is below; the other three
were omitted from this entry when it was first written, which made them look
unreleased to anyone reading the changelog rather than the API.
...
Every decision made through the approvals path has been observed since this
shipped, so the shadow window is already accumulating — the report is a read
over data that is being collected now, not something that starts when a UI
appears. Promotion refuses entirely while the org is in shadow mode.

A promoted rule covers only what was actually approved: it is bounded by the
amount band it was earned in, and a run of approvals for a familiar recipient
never licenses a new destination. A profile promotes onto the agent whose
approvals built it and no other.
...
Applying a preset that loosens a guardrail returns 202 or 403, never a silent
200: a preset builds the same request a person editing by hand would send and
passes it to the same handler, so it is a friendlier interface to the approval
flow rather than a way around it. Preview names every field that would widen.
See [Policy presets](/docs/agents/policy-presets).
...
See [Trust signals](/docs/agents/trust-signals).
...
- New CRUD endpoints: `POST/GET/DELETE /v1/treasury/wallets/access-policies`
...
- **Agent Discovery** — Public agent directory and platform marketplace. Agents can be made discoverable with capability cards showing A2A/MCP URLs, supported protocols, and pricing. Endpoints: `GET /v1/agents/directory`, `GET /v1/agents/{agent_id}/card`, `POST /v1/agents/{agent_id}/discovery`. SDK `client.discovery.*`, CLI `1claw directory`, MCP `search_directory`. Dashboard: `/directory` public page, discovery card on agent detail.
