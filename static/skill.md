...
**Extended inspection pipeline** — Beyond the threat detectors above, `shroud_config` also supports: `tool_call_inspection` (scan function/tool call arguments, block credential exfiltration — `block_credential_exfil` defaults to **`true`**), `output_policy` (block harmful content, patterns, and entities in LLM responses), `secret_injection_detection` (detect secrets injected into prompts/responses), `advanced_redaction` (detect base64-encoded, split, or prefix-leaked secrets), and `semantic_policy` (enforce allowed/denied topics and tasks at the intent level). `flagged_request_retention_days` controls how long flagged requests are retained. `streaming_inspection_mode` (`rolling` default: text released behind a tail buffer sized to the org's longest secret, tool-call deltas held until complete, injection heuristics warn-only; `holdback`: `streaming_holdback_chars` buffered, injection blocks) and `max_concurrent_streams` (default 20, `429` beyond) govern `stream: true`. Dashboard: `/shroud-activity` (overview), `/shroud-activity/threats`, `/shroud-activity/live` (real-time inspector).

**Router rail (v0.61.38–42)** — A stock OpenAI/Anthropic SDK can talk to Shroud unmodified: a human mints an `sk-shroud-v1-<32>` router key (`POST /v1/agents/{id}/router-keys`; CLI `1claw agent create-router-key`; SDK `agents.createRouterKey`), the SDK sends it as `Authorization: Bearer` to `https://shroud.1claw.co/v1` with `X-Shroud-Provider`, and the gateway exchanges it for a ≤ 60 s agent token (`DELETE …/router-keys/{key_id}` lands within a minute). Lone `ocv_`, `agent_id:ocv_` and agent JWTs still work. Router-key traffic pays **$0.005 per inspected request** from the org's micro-USD ledger (`GET /v1/billing/credits/balance` → `balance_micro_usd`; card top-up `POST /v1/billing/credits/topup`, or `Authorization: Bearer x402` → 402 quote → `X-PAYMENT` for USDC on Base; an unknown wallet's first settled payment provisions its own org + agent + router key); bodies > 1 MB → 413; an in-flight stream is never cut for billing. Streaming is inspected per frame (`x-shroud-stream-inspection: per-frame`). With the attestation-released enclave key, vault secrets in prompts become `⟦sk:type:tagck⟧` placeholders the model can place but never read; they are rehydrated only inside the enclave, only where a **tool binding** allows it (`POST /v1/agents/{id}/tool-bindings`: `secret_path`, `tool_name`, `arg_path`, `destination_hosts`; CLI `1claw agent bind-secret`; SDK `agents.createToolBinding`) and only toward the bound hosts. Docs: https://docs.1claw.co/docs/agents/shroud/overview#auth.
